...
def perform_asr(
    ...
    **kwargs,
    ...
    if asr_api == "nemo":
        ...
        return perform_asr_nemo(
            audio, pipe, timestamps=True, timestamp_level=timestamp_level
        )
...
def perform_asr_nemo(
    ...
    **kwargs,
    ...
    result = {"chunks": result.timestamp[timestamp_level], "text": result.text}
...
def load_and_resample(fp: str, sr: int = SAMPLE_RATE) -> torch.Tensor:
    wav_orig, sr_orig = librosa.load(fp)
    wav = librosa.resample(y=wav_orig, orig_sr=sr_orig, target_sr=sr)
    wav = torch.tensor(wav)
    if wav.dim() > 1:
        return wav[:1, :]
    return wav.unsqueeze(dim=0)
...
def init_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        "-i", "--input", help=".wav file or directory of .wav files to annotate"
    )
...
def annotate(args) -> int:
    ...
    if os.path.isdir(args.input):
        ...
        glob_str_upper = glob_str.replace(
            args.file_extension, args.file_extension.upper()
        )
        audio_fps = glob(glob_str, recursive=args.recursive) + glob(
            glob_str_upper, recursive=args.recursive
        )
        for audio_fp in audio_fps:
            out_txt = change_filepath(
                media_fp=audio_fp, ext=".txt", new_dir=args.output
            )
...
def drz_only(
    ...
    diarization = diarize(wav, drz_pipe, num_speakers=num_speakers).speaker_diarization
...
def asr_first(
    ...
    **kwargs,
    ...
    gecko_json = {"schemaVersion": 2.0, "monologues": []}
    speakers = diarization.labels()
    for speaker in speakers:
        eaf.add_tier(speaker)
